...
- **Course catalog sync (belongs in Task 3, added post-hoc):** `src/lib/courseCatalog.ts` hardcodes each course's `minutes` and `modules` totals, and `src/lib/__tests__/courseCatalog.test.ts` asserts they equal the summed definition. Adding subchapter 3.2 (`minutes: 7`) raises the attention course's minute total, so the catalog entry's `minutes` must go **106 → 113** (module count stays 7 — subchapters don't count). Also refresh the free-text `highlights` for accuracy: `14 interactive labs · 7 deep dives` → `15 interactive labs · 8 deep dives` (one new lab = MlaLab, one new deep dive = 3.2). This test lives in `src/lib/`, outside the `src/components/courses/attention/` scope the task's own test runs used — hence it was missed until the full-suite verification.
